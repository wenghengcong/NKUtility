...
public protocol NKContactsPickerDelegate: class {
    ...
    func contactPicker(_: NKContactsPicker, didContactFetchFailed error: NSError)
    ...
    func contactPickerDidCancel(_: NKContactsPicker)
    
    /// Called when a contact is selected.
    /// - Note: This method is called when `multiSelectEnabled` is `false`. If `multiSelectEnabled` is `true` then
    /// `contactPicker(_: ContactsPicker, didSelectMultipleContacts contacts: [CNContact])` is called instead.
    ///
    /// - Parameters:
    ///   - _: The `contactPicker` instance which called this method.
    ///   - contact: The `CNContact` that was selected by the user.
    func contactPicker(_: NKContactsPicker, didSelectContact contact: CNContact)
    ...
    /// `func contactPicker(_: ContactsPicker, didSelectContact contact: CNContact)` is called instead.
    ///
    /// - Parameters:
    ///   - _: The `contactPicker` instance which called this method.
    ///   - contacts: An array of `CNContact`'s selected by the user.
    func contactPicker(_: NKContactsPicker, didSelectMultipleContacts contacts: [CNContact])
}

public extension NKContactsPickerDelegate {
    func contactPicker(_: NKContactsPicker, didContactFetchFailed error: NSError) { }
    func contactPickerDidCancel(_: NKContactsPicker) { }
    func contactPicker(_: NKContactsPicker, didSelectContact contact: CNContact) { }
    func contactPicker(_: NKContactsPicker, didSelectMultipleContacts contacts: [CNContact]) { }
...
open class NKContactsPicker: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate, NKContactPickerCellDelegate {
    ...
    public private(set) var selectedContacts = [CNContact]()

    private var filteredContacts = [CNContact]()
    
    /// The `delegate` for the picker.
    open weak var contactDelegate: NKContactsPickerDelegate?
    ...
    public lazy var searchBar: UISearchBar = {
        ...
        searchBar.placeholder = NKContactConstants.Strings.searchBarPlaceHloder
    ...
    public init(delegate: NKContactsPickerDelegate? = nil,
    ...
    override open func viewDidLoad() {
        super.viewDidLoad()
        self.title = NKContactConstants.Strings.contactsTitle
    ...
    private func registerContactCell() {
        let cellNib = UINib(nibName: NKContactConstants.Strings.cellNibIdentifier, bundle: nil)
        tableView.register(cellNib, forCellReuseIdentifier: NKContactConstants.Strings.cellNibIdentifier)
        
        // Lib_TODO: bundle
//        let podBundle = Bundle(for: self.classForCoder)
//        if let bundleURL = podBundle.url(forResource: NKContactPickerConstants.Strings.bundleIdentifier, withExtension: "bundle") {
//            if let bundle = Bundle(url: bundleURL) {
//                let cellNib = UINib(nibName: NKContactPickerConstants.Strings.cellNibIdentifier, bundle: bundle)
//                tableView.register(cellNib, forCellReuseIdentifier: "NKContactPickerCell")
//            }
//            else {
...
//
//        } else {
//            let cellNib = UINib(nibName: NKContactPickerConstants.Strings.cellNibIdentifier, bundle: podBundle)
//            tableView.register(cellNib, forCellReuseIdentifier: "Cell")
//        }
    }
    
    // MARK: - CNContact Operations
    ...
    private func getContacts(_ completion:  @escaping ContactsHandler) {
        ...
        let error = NSError(domain: "NKContactPickerErrorDomain", code: 1, userInfo: [NSLocalizedDescriptionKey: "No Contacts Access"])
        ...
        case CNAuthorizationStatus.notDetermined:
            ...
            NKContactService.shared.requestAcces { (granted, error) in
    ...
    func allowedContactKeys() -> [CNKeyDescriptor]{
        ...
        return NKContactConstants.FetchKeys.brief
    }
    ...
    open func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "NKContactPickerCell", for: indexPath) as! NKContactPickerCell
        ...
        if multiSelectEnabled  && selectedContacts.contains(where: { $0.identifier == contact.identifier }) {
    ...
    open func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let cell = tableView.cellForRow(at: indexPath) as! NKContactPickerCell
        ...
        if multiSelectEnabled {
            ...
            if cell.accessoryType == UITableViewCell.AccessoryType.checkmark {
                ...
                selectedContacts = selectedContacts.filter(){
                    return selectedContact.identifier != $0.identifier
                }
    ...
    public func clickInfo(cell: NKContactPickerCell, contact: CNContact?) {
        ...
        do {
            let dcontacts = try self.contactsStore.unifiedContact(withIdentifier: (contact!.contactId!), keysToFetch: [CNContactViewController.descriptorForRequiredKeys()])
    ...
    private func updateContacts(with predicate: NSPredicate) {
        do {
            ...
            contactDelegate?.contactPicker(self, didContactFetchFailed: NSError(domain: "NKContactsPickerErrorDomain",
                                                                                code: 3,
