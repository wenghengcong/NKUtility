//
//  NKPlaceholderDataSourceDelegate.swift
//  Pods
//
...
class NKPlaceholderDataSourceDelegate: NSObject {
    
    // MARK: properties
    
    /// The placeholder object
    let placeholder: NKPlaceholder
    
    // MARK: init methods

    /// Create and return a NKPlaceholderDataSourceDelegate object with the specified NKPlaceholder
    ///
    /// - Parameter placeholder: the placeholder object
    init(placeholder: NKPlaceholder) {
    ...
    ///
    /// - Parameters:
    ///   - cell: the cell can be an UITableViewCell or UICollectionViewCell, but it must be conform to the protocol NKCellPlaceholding
    ///   - placeholder: the placeholder object
    ///   - tintColor: the tint color to apply
    func fill(cell: NKCellPlaceholding, to placeholder: NKPlaceholder, tintColor: UIColor?) {
    ...
    func animate(cell: NKCellPlaceholding) {
 ...
 */
extension NKPlaceholderDataSourceDelegate: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let reuseIdentifier = placeholder.cellIdentifier ?? NKPlaceholderTableViewCell.reuseIdentifier
        ...
        // If the cell does not inherit from NKPlaceholderTableViewCell, the data and the style can't be applied
        guard let placeholderTableViewCell = cell as? NKPlaceholderTableViewCell else {
            return cell
        }
        ...
        placeholderTableViewCell.onActionButtonTap = { [unowned self] in
            guard let placeholderTableView = (tableView as? NKPlaceholderTableView) else { return }
 ...
 */
extension NKPlaceholderDataSourceDelegate: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        guard let placeholderTableViewCell = cell as? NKPlaceholderTableViewCell else { return }
...
extension NKPlaceholderDataSourceDelegate: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let reuseIdentifier = placeholder.cellIdentifier ?? NKPlaceholderCollectionViewCell.reuseIdentifier
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: reuseIdentifier, for: indexPath)

        // If the cell does not inherit from NKPlaceholderTableViewCell, the data and the style can't be applied
        guard let placeholderCollectionViewCell = cell as? NKPlaceholderCollectionViewCell else {
            return cell
        }
        ...
        placeholderCollectionViewCell.onActionButtonTap = { [unowned self] in
            guard let placeholderCollectionView = collectionView as? NKPlaceholderCollectionView else { return }
...
extension NKPlaceholderDataSourceDelegate: UICollectionViewDelegateFlowLayout {
