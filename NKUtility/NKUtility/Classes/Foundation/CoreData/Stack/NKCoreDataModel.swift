...
/**
 An instance of `NKCoreDataModel` represents a Core Data model — a `.xcdatamodeld` file package.
 It provides the model and store URLs as well as methods for interacting with the store.
 */
public struct NKCoreDataModel {
    ...
    public let storeType: NKStoreType
    ...
    /**
     Constructs a new `NKCoreDataModel` instance with the specified name and bundle.
     ...
     - returns: A new `NKCoreDataModel` instance.
     */
    public init(name: String, bundle: Bundle = .main, storeType: NKStoreType = .sqlite(defaultDirectoryURL())) {
        self.name = name
        self.bundle = bundle
    ...
    /// The default directory used to initialize a `NKCoreDataModel`.
...
extension NKCoreDataModel: Equatable {
    /// :nodoc:
    public static func == (lhs: NKCoreDataModel, rhs: NKCoreDataModel) -> Bool {
        lhs.name == rhs.name
