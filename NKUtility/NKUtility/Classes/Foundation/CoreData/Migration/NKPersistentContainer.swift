...
protocol NKPersistentContainerProtocol: NKPersistentContainerMigratable {
...
/// Pull out the front-end code shared between `NKPersistentContainer` and `NKPersistentCloudKitContainer`.
@available(macOS 10.12, iOS 10.0, tvOS 10.0, watchOS 3.0, *)
extension NKPersistentContainerProtocol {
...
///
/// See `NKPersistentCloudKitContainer` for a version capable of managing both
/// CloudKit-backed and non-cloud stores..
///
@available(macOS 10.12, iOS 10.0, tvOS 10.0, watchOS 3.0, *)
open class NKPersistentContainer: NSPersistentContainer, NKPersistentContainerMigratable, NKPersistentContainerProtocol, LogMessageEmitter {

    /// Background queue for running store operations.
    let dispatchQueue = DispatchQueue(label: "NKPersistentContainer", qos: .utility)

    /// User's model version order.
    let modelVersionOrder: NKCDModelVersionOrder

    /// List of bundles to search for Core Data models.
    let bundles: [Bundle]

    /// Core Data model version graph discovered from the bundles.
    var modelVersionGraph: NKCDModelVersionGraph?

    /// Optional delegate to receive migration progress notifications.
    public weak var migrationDelegate: NKCDMigrationDelegate?
    ...
    public convenience init(name: String,
                            ...
                            modelVersionOrder: NKCDModelVersionOrder = .compare,
