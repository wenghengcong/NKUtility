...
struct NKCDModelVersionNode: NamedEntityVersion, Equatable, CustomStringConvertible {
    let name: String
    ...
    static func ==(lhs: NKCDModelVersionNode, rhs: NKCDModelVersionNode) -> Bool {
...
final class ModelVersionNodes: LogMessageEmitter {
    ...
    private var nodesByName: [String:NKCDModelVersionNode]

    var nodes: [NKCDModelVersionNode] {
        return Array(nodesByName.values)
    }

    let logMessageHandler: LogMessage.Handler?

    init(nodesByName: [String:NKCDModelVersionNode] = [:], logMessageHandler: LogMessage.Handler?) {
        self.nodesByName = nodesByName
        self.logMessageHandler = logMessageHandler
    }

    subscript(nodeName: String) -> NKCDModelVersionNode? {
    ...
    func discoverCandidates(from bundles: [Bundle]) -> [NKCDModelVersionNode] {
        ...
        var modelVersionNodes: [NKCDModelVersionNode] = []

        modelEntityVersions.forEach { modelDirURL, modelEntityVersions in

            let modelDirNodes = modelEntityVersions.compactMap { versionName, entityVersions -> NKCDModelVersionNode? in
                ...
                return NKCDModelVersionNode(name: versionName, model: model)
            }
    ...
    func filtered(order: NKCDModelVersionOrder) -> ModelVersionNodes {
