...
/// `NKPersistentContainer.loadPersistentStores(...)` or
/// `NKPersistentCloudKitContainer.loadPersistentStores(...)`.
///
...
public enum NKCDMigrationError: Error {
    ...
    /// Migration cannot precede because the `NKCDModelVersionOrder` for the store is invalid.
    /// This could mean:
    ///
    ...
    case badModelVersionOrder(NSPersistentStoreDescription, NKCDModelVersionOrder)
    ...
    /// `NKPersistentContainer.init(...)` or `NKPersistentCloudKitContainer.init(...)`.
    ...
    /// that matches the `NSManagedObjectModel` passed to `NKPersistentContainer.init(...)`
    /// or `NKPersistentCloudKitContainer.init(...)` in the list of bundles.
    ...
    /// parameter) using mapping models from the bundles passed to `NKPersistentContainer.init(...)`
    /// or `NKPersistentCloudKitContainer.init(...)` combined with inferred mappings if enabled.
    /// Possible reasons include:
    ///
    ///  * The `NKCDModelVersionOrder` given to the container is too strict/incorrect;
    ...
    /// This probably means there are cycles in the persistent container's `NKCDModelVersionOrder`.
    ...
    /// of migrating between the two model versions named in the string parameters.
    /// This also probably means there are cycles in the persistent container's `NKCDModelVersionOrder`.
...
@available(macOS 10.12, iOS 10.0, tvOS 10.0, watchOS 3.0, *)
extension NKCDMigrationError: CustomStringConvertible {
