...
## 1. Declare your cells to conform to `Reusable` or `NKUINibReusable`

* Use the `Reusable` protocol if they don't depend on a NIB (this will use `registerClass(…)` to register the cell)
* Use the `NKUINibReusable` typealias (= `Reusable & NibLoadable`) if they use a `XIB` file for their content (this will use `registerNib(…)` to register the cell)
...
> * 💡 `NKUINibReusable` is a typealias, so you could still use two protocols conformance `Reusable, NibLoadable` instead of `NKUINibReusable`.
...
```swift
final class NibBasedCustomCell: UITableViewCell, NKUINibReusable {
// or
...
final class NibBasedCollectionViewCell: UICollectionViewCell, NKUINibReusable {
// or
...
class MyViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    // because the CodeBasedCustomCell type conforms to Reusable, but not NibLoadable (nor the NKUINibReusable typealias)
    ...
    // because the NibBasedCustomCell type conforms to NibLoadable (via the NKUINibReusable typealias)
...
The protocols in this pod, like `Reusable`, `NibLoadable`, `NibOwnerLoadable`, `StoryboardBased`, `NKUINibReusable`… are what is usually called [Mixins](http://alisoftware.github.io/swift/protocol/2015/11/08/mixins-over-inheritance/), which basically is a Swift protocol with a default implementation provided for all of its methods.
...
```swift
final class VeryCustomNibBasedCell: UITableViewCell, NKUINibReusable {
  // This cell use a non-standard configuration: its reuseIdentifier and XIB file
  // have a different name as the class itself. So we need to provide a custom implementation or `NKUINibReusable`
