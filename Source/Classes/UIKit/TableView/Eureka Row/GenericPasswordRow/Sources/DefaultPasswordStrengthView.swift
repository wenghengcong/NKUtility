...
open class DefaultPasswordStrengthView: PasswordStrengthView {
    ...
    open var rowBorderColor = UIColor.lightGray.withAlphaComponent(0.2)
    open var rowBorderWidth = CGFloat(1)
    open var rowCornerRadius = CGFloat(3)
    open var rowAnimationTime = TimeInterval(0.3)
    ...
    override open func setPasswordValidator(_ validator: PasswordValidator) {
        ...
        strengthViews = colorsForStrenghts.enumerated().map { index, element in
            let view = UIView()
            view.layer.borderColor = rowBorderColor.cgColor
            view.layer.borderWidth = rowBorderWidth
    ...
    open func setup() {
        clipsToBounds = true
        layer.cornerRadius = rowCornerRadius
        progressView = UIView()
        progressView.layer.borderColor = rowBorderColor.cgColor
        progressView.layer.borderWidth = rowBorderWidth
    ...
    open func updateView(animated: Bool) {
        setNeedsLayout()
        if animated {
            UIView.animate(withDuration: rowAnimationTime, animations: { [weak self] in
                self?.layoutIfNeeded()
            }, completion: { [weak self] _ in
                UIView.animate(withDuration: self?.rowAnimationTime ?? 0.3, animations: { [weak self] in
