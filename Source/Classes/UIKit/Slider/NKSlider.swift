...
open class NKSlider : UIControl {
    ...
    open var didBeginTracking: ((NKSlider) -> ())?
    open var didEndTracking: ((NKSlider) -> ())?
	...
	open var valueViewMargin: CGFloat = NKSliderValueView.kLayoutMarginInset {
		didSet {
			if valueViewMargin < NKSliderValueView.kLayoutMarginInset {
				valueViewMargin = NKSliderValueView.kLayoutMarginInset
			}
			layoutValueView()
		}
	}
    
    private let valueView = NKSliderValueView()
	...
	private(set) open var isNKSliderTracking = false
    
    private func updateValueViewText() {
		if !showFractionOnlyWhileTracking || isNKSliderTracking {
	...
	private func layoutImageViews() {
		let imageInset = NKSliderValueView.kLayoutMarginInset * 2
    ...
    private func boundsForValueViewCenter() -> CGRect {
        return bounds.inset(by: UIEdgeInsets(top: 0, left: valueViewMargin - NKSliderValueView.kLayoutMarginInset + valueView.bounds.midX, bottom: 0, right: valueViewMargin - NKSliderValueView.kLayoutMarginInset + valueView.bounds.midX))
    }
    ...
    private let filter = NKSliderMetaballFilter()
