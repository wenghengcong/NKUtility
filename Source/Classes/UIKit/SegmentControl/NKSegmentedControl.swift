...
@IBDesignable open class NKSegmentedControl: UIControl {
    ...
    public var segments: [NKSegmentedControlSegment] {
        didSet {
            applySegments()
        }
    }
    ...
    // MARK: Initialization
    /// Initializes a new `NKSegmentedControl` with the parameters passed.
    ///
    /// - Parameters:
    ///   - frame: The frame of the control.
    ///   - segments: The segments to configure the control with.
    ///   - index: The initially selected index. Passing `NKSegmentedControl.noSegment` sets the index to `-1` and hides the
    ...
    public init(frame: CGRect,
                segments: [NKSegmentedControlSegment],
                index: Int = 0,
                options: [Option]? = nil) {
        ...
        setOptions(NKSegmentedControl.defaultOptions)
    ...
    ///
    /// - Parameters:
    ///   - index: The new index. Passing `NKSegmentedControl.noSegment` sets the index to `-1` and hides the indicator view.
    ...
    private func frameForElement(atIndex index: Int) -> CGRect {
        let elementWidth = (self.bounds.width - totalInsetSize) / CGFloat(normalSegmentViewCount)
        let x = CGFloat(isLayoutDirectionRightToLeft ? lastIndex - index : index) * elementWidth
        
        return CGRect(x: x + indicatorViewInset,
                      y: indicatorViewInset,
                      width: elementWidth,
                      height: self.bounds.height - totalInsetSize)
    }
    ...
    private static func generateDefaultSegments() -> [NKLabelSegment] {
...
extension NKSegmentedControl: UIGestureRecognizerDelegate {
